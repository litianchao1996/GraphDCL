...
from Graph_Encoder import Node_Aggregator, Graph_Encoder, Node_Encoder, to_adj_matrix, to_index_attr

from Graph_Augmentation import Permutation_Aug, Transformation_Aug
...
def train_encoder(encoder, optimizer, loss_fn, alpha, num_epochs, data_loader, batch_size, left, GCN=False):
    ...
    for epoch in range(num_epochs):
        ...
        for step, batch in enumerate(data_loader):
            ...
            if step >= total_step:
                ...
                for i in range(batch_size):
                    ...
                    p_x1, p_adj1, _ = p_list[0]
                    p_x2, p_adj2, _ = p_list[1]
                    p_edge_index1, p_edge_attr1 = to_index_attr(p_adj1)
                    p_edge_index2, p_edge_attr2 = to_index_attr(p_adj2)
                    #p_adj1, p_adj2 = p_adj1.to(device), p_adj2.to(device)
                    p_x1, p_x2 = p_x1.to(device), p_x2.to(device)
                    ...
                    t_x1, t_adj1, orth1 = t_list[0]
                    t_x2, t_adj2, orth2 = t_list[1]
                    t_edge_index1, t_edge_attr1 = to_index_attr(t_adj1)
                    t_edge_index2, t_edge_attr2 = to_index_attr(t_adj2)

                    #orth1, orth2 = orth1.to(device), orth2.to(device)
                    #t_adj1, t_adj2 = t_adj1.to(device), t_adj2.to(device)
                    t_x1, t_x2 = t_x1.to(device), t_x2.to(device)
                    ...
                    if not GCN:
                        ...
                        t_z1 = encoder(t_x1, t_adj1).to(device)
                        t_z1_list.append(p_z1)
                   
                        t_z2 = encoder(t_x2, t_adj2).to(device)
        ...
        model_save_name = 'alpha5translstmNCI1.pt'
...
if __name__ == "__main__":
    ...
    GCN = False
    logger.info('alpha5translstmNCI1.pt')
    node_encoder = Node_Encoder(37, 256, 'trans', 1)
    aggregator = Node_Aggregator(256, 1024, 'lstm')
    graph_encoder = Graph_Encoder(1024, 300, node_encoder, aggregator)
    #model_save_name = 'alpha2.5nnconvlstmNCI1.pt'
    #path = F"./save/{model_save_name}" 
